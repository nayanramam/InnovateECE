...
module IF_Stage (
  ...
  always @(*) begin
    ...
    if (reset == 1'b1) //reset
      Next_PC = 0;
    else if (flush_en == 1'b1) //when flush, we get our next pc from the alu branch/jump
      Next_PC = alu_result_valid_ip ? alu_result_ip : pc_addr;
    else if (stall_ip == 1'b1) //when stalled, just stay at current pc
      Next_PC = pc_addr;
    else begin //normal fetch
