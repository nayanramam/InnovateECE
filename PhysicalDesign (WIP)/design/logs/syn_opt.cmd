...
if {[catch {init_flow  {flow_script {} yaml_script /nethome/dkhalil8/InnovateECE/PhysicalDesign/design/scripts/flow.yaml flow_no_check 0 parent_uuid {} previous_uuid {} top_dir /nethome/dkhalil8/InnovateECE/PhysicalDesign/design flow_dir . status_file /nethome/dkhalil8/InnovateECE/PhysicalDesign/design/flow.status.d/syn_opt metrics_file /nethome/dkhalil8/InnovateECE/PhysicalDesign/design/flow.metrics.d/syn_opt run_tag {} db {rc /nethome/dkhalil8/InnovateECE/PhysicalDesign/design/dbs/syn_map.db Core {}} db_is_ref_run 0 branch {} caller_data {group 0 process_branch 0 trunk_process 1 flowtool_hostname ece-rschsrv.ece.gatech.edu flowtool_pid 1311060} flow {flow flow:flow_current dir . db {rc dbs/syn_map.db Core {}} branch {} tool genus caller_data {group 0 process_branch 0 trunk_process 1 flowtool_hostname ece-rschsrv.ece.gatech.edu flowtool_pid 1311060} uuid {} tool_options {} start_step {tool genus flow flow:flow_current canonical_path {.steps flow:synthesis .steps flow:syn_opt .steps flow_step:block_start} step flow_step:block_start features {} str synthesis.syn_opt.block_start} process_branch_trunk 1} flow_name flow:flow_current first_step {tool genus flow flow:flow_current canonical_path {.steps flow:synthesis .steps flow:syn_opt .steps flow_step:block_start} step flow_step:block_start features {} str synthesis.syn_opt.block_start} interactive 0 interactive_run 0 enabled_features {} inject_tcl {} trunk_process 1 aum_upload false tool_options {} overwrite 0 last_step {tool genus flow flow:flow_current canonical_path {.steps flow:synthesis .steps flow:syn_opt .steps flow_step:schedule_syn_opt_report_synth} step flow_step:schedule_syn_opt_report_synth features {} str synthesis.syn_opt.schedule_syn_opt_report_synth} log_prefix /nethome/dkhalil8/InnovateECE/PhysicalDesign/design/logs/syn_opt}; run_flow -from {tool genus flow flow:flow_current canonical_path {.steps flow:synthesis .steps flow:syn_opt .steps flow_step:block_start} step flow_step:block_start features {} str synthesis.syn_opt.block_start} -to {tool genus flow flow:flow_current canonical_path {.steps flow:synthesis .steps flow:syn_opt .steps flow_step:schedule_syn_opt_report_synth} step flow_step:schedule_syn_opt_report_synth features {} str synthesis.syn_opt.schedule_syn_opt_report_synth}} msg]} { puts [concat {Tcl error:} $errorInfo]; set fp [open {/nethome/dkhalil8/InnovateECE/PhysicalDesign/design/flow.status.d/syn_opt} a]; puts $fp {}; puts $fp [list [list script run_tcl status error flow {flow:flow_current} branch {} flow_working_directory {.} flow_starting_db {rc /nethome/dkhalil8/InnovateECE/PhysicalDesign/design/dbs/syn_map.db Core {}} {tool_options} {} steps_run [get_db flow_step_canonical_current] msg $msg]]; close $fp; exit 1 }; exit 0
#@ (init_flow): cd /nethome/dkhalil8/InnovateECE/PhysicalDesign/design
#@ (init_flow): read_metric -id current /nethome/dkhalil8/InnovateECE/PhysicalDesign/design/flow.metrics.d/syn_opt -previous 
...
#@ (init_flow): read_flow /nethome/dkhalil8/InnovateECE/PhysicalDesign/design/scripts/flow.yaml
#@ (init_flow): cd /nethome/dkhalil8/InnovateECE/PhysicalDesign/design
#@ (init_flow): read_metric -merge -id current /nethome/dkhalil8/InnovateECE/PhysicalDesign/design/flow.metrics.d/syn_opt -previous 
#@ (flow_step:block_start)  2:   apply {{} {
#@                           :     set db [get_db flow_starting_db]
#@                           :     set flow [lindex [get_db flow_hier_path] end]
#@                           :     set setup_views [get_feature -obj $flow setup_views]
...
#@ (flow_step:block_start)  3:   # Only apply in Innovus when the input is a Genus synthesis flat DB (rc).
#@ (flow_step:block_start)  4:   # Genus blocks also read rc DBs but are at initialization_complete already,
#@ (flow_step:block_start)  5:   # so we guard with program_short_name to avoid TUI-340 in Genus.
#@ (flow_step:block_start)  6:   if {[string match "innovus*" [get_db program_short_name]] && [dict exists [get_db flow_starting_db] rc]} {
...
#@ (run_flow): pop_snapshot_stack
#@ (flow_step:run_syn_opt)  2:   #- extend flow report name based on context
#@ (flow_step:run_syn_opt)  3:   if {[is_flow -quiet -inside flow:sta] || [is_flow -quiet -inside flow:sta_dmmmc] || [is_flow -quiet -inside flow:sta_eco]} {
#@                           :     if {![regexp {sta$} [get_db flow_report_name]]} {
#@                           :       set_db flow_report_name [expr {[string is space [get_db flow_report_name]] ? "sta" : "[get_db flow_report_name].sta"}]
#@                           :     }
#@                           :   } elseif {[is_flow -quiet -inside flow:ir_early_static] || [is_flow -quiet -inside flow:ir_early_dynamic]} {
...
#@                           :   } else {
#@                           :   }
#@ (flow_step:run_syn_opt) 22:   #- Create report dir (if necessary)
#@ (flow_step:run_syn_opt) 23:   file mkdir [file normalize [file join [get_db flow_report_directory] [get_db flow_report_name]]]
#@ (run_flow): push_snapshot_stack
...
#@                            :   }}
#@ (run_flow): pop_snapshot_stack
#@ (run_flow): write_db -all_root_attributes -to_file /nethome/dkhalil8/InnovateECE/PhysicalDesign/design/dbs/syn_opt.db
#@ (run_flow): push_snapshot_stack
